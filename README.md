# 🚀 TaskFlow IA - Gerenciador de Tarefas Interativo com IA

[![Status: Prototipação Funcional](https://img.shields.io/badge/Status-Prototipação%20Funcional-blue.svg)](URL_DO_PROJETO_EM_PRODUCAO)

🔗 **Links Relevantes:**
- [Acesso ao Deploy](URL_DO_PROJETO_EM_PRODUCAO)
- https://docs.n8n.io/workflows/(URL_DO_FLUXO_N8N_EXPOSTO)

Um template de aplicação full-stack que demonstra a integração de IA conversacional (**Gemini**) para gerenciamento de tarefas (CRUD) via chat. Utiliza **n8n** como camada de orquestração backend e **Supabase** para persistência de dados e autenticação (OAuth).

## ✨ Motivação e Escopo (O Custo e a Oportunidade)

### O Problema Abordado
O principal desafio é traduzir a **linguagem natural** do usuário (via Gemini) em **ações transacionais** no banco de dados (CRUD de tarefas) de forma segura e com contexto.

### Objetivo Principal
Criar uma **base arquitetural desacoplada** onde a inteligência é provida por IA, o fluxo de trabalho é gerido por low-code (n8n), e o backend é consolidado pelo Supabase.

### Custos de Oportunidade e Limites
* **Performance vs. Agilidade:** A utilização do n8n (orquestração) e da API free tier do Gemini introduz latência. O custo de oportunidade é em **tempo de resposta** em troca da **agilidade na prototipação**.
* **Segurança Delegada:** Ao usar a chave de Service Role do Supabase no n8n, a responsabilidade de **autorização de dados por usuário** é transferida do Supabase (RLS) para o **fluxo lógico do n8n**. O rigor neste fluxo é **crítico** para a segurança.

## 🛠️ Tecnologias Chave

| Tecnologia | Função no Projeto |
| :--- | :--- |
| **Gemini API** | **Inteligência Conversacional:** Analisa o texto do usuário e gera um payload estruturado (JSON) para ações CRUD. |
| **n8n** | **Orquestração Low-Code:** Atua como o backend intermediário, gerindo o histórico e executando as operações de CRUD no Supabase. |
| **Supabase (PostgreSQL + Auth)** | **Backend Unificado:** Consolida a base de dados relacional (PostgreSQL) e a infraestrutura para a autenticação OAuth. |
| **OAuth 2.0 (via Supabase)** | **Controle de Acesso:** Garante que cada usuário gerencie apenas seu próprio conjunto de tarefas e histórico. |

-----

## 💻 Como Rodar Localmente

Siga estas etapas para configurar e rodar o projeto na sua máquina:

### 1\. Pré-requisitos

Certifique-se de ter instalado:

* Node.js (versão recomendada)
* npm, yarn ou pnpm
* Uma conta ativa no **Supabase**.

### 2\. Clone o Repositório

```bash
git clone [https://docs.github.com/pt/migrations/importing-source-code/using-the-command-line-to-import-source-code/adding-locally-hosted-code-to-github](https://docs.github.com/pt/migrations/importing-source-code/using-the-command-line-to-import-source-code/adding-locally-hosted-code-to-github)
cd taskflow-ia
````

### 3\. Instale as Dependências

```bash
npm install
# ou
yarn install
```

### 4\. Configuração das Variáveis de Ambiente

Crie um arquivo chamado **`.env.local`** na raiz do projeto e adicione suas chaves:

```
# .env.local

# URL da sua aplicação
NEXTAUTH_URL="http://localhost:3000"
NEXTAUTH_SECRET="gere um codigo seguro"

# N8N Credentials
N8N_WEBHOOK_URL="SUA URL DO WEBHOOK N8N"
N8N_AUTH_TOKEN="CHAVE DE AUTENTICAÇÃO PARA BLOCO DO N8N"

# Supabase Credentials
SUPABASE_PROJECT_URL="SUA URL SUPABASE"
SUPABASE_API_KEY=SUA SECRET  KEY SUPABASE

NEXT_PUBLIC_SUPABASE_PROJECT_URL="SUA URL SUPABASE"
NEXT_PUBLIC_SUPABASE_API_KEY=SUA KEY SUPABASE

# Google OAuth Credentials
GOOGLE_CLIENT_ID=SEU GOOGLE CLIENT ID
GOOGLE_CLIENT_SECRET=SEU GOOGLE SECRET

```

### 5\. Configuração do Banco de Dados (Supabase)

É **mandatório** que o esquema exista antes de iniciar a aplicação.

#### 5.1. Criação das Tabelas

Execute o seguinte script SQL no SQL Editor do Supabase:

```sql
-- Tabela de Usuários (necessária para Auth)
CREATE TABLE users (
    id TEXT PRIMARY KEY,
    email TEXT UNIQUE NOT NULL,
    name TEXT,
    image TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now())
);

-- Tabela para Itens de Tarefa
CREATE TABLE todo_items (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id TEXT REFERENCES users(id) NOT NULL,
    task_description TEXT NOT NULL,
    title TEXT,
    category TEXT,
    is_completed BOOLEAN DEFAULT FALSE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now())
);

-- Tabela para Histórico de Conversação (Contexto do Chat)
CREATE TABLE chat_history (
    id TEXT PRIMARY KEY,
    user_id TEXT REFERENCES users(id) NOT NULL,
    conversation JSONB NOT NULL, 
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now())
);
```

#### 5.2. Configuração do n8n

1. Inicie o docker usando "docker compose up -d" 
2. Abra o localhost do n8n e crie uma conta
3. Importe o "My workflow" que vem junto com os arquivos do projeto

### 6\. Inicie o Servidor de Desenvolvimento

```bash
npm run dev
# ou
yarn dev
```

Abra `http://localhost:3000` no seu navegador.
